...
class Llms:
    ...
    def __init__(self):
        ...
        self.available_llms = ["anthropic.claude-instant-v1", "anthropic.claude-v2", "anthropic.claude-v2:1",  "anthropic.claude-3-sonnet-20240229-v1:0", "anthropic.claude-3-haiku-20240307-v1:0", "anthropic.claude-3-opus-20240229-v1:0", "anthropic.claude-3-5-sonnet-20240620-v1:0", "llama3-local","llama3-local-ft", "meta.llama3-1-405b-instruct-v1:0", "meta.llama3-1-70b-instruct-v1:0", "meta.llama3-1-8b-instruct-v1:0"]

    def load_local_llm(self, load_ft_model):
        from llms_ft import FT_Llama
        self.ftl = FT_Llama()
        if load_ft_model:
            self.ftl.load_ft_model()
    ...
    def ask(self, prompt, model_id):
        if model_id not in self.available_llms:
            ...
            return self.ftl.ask(prompt)
        elif model_id == "llama3-local-ft":
            return self.ftl.ask(prompt)
    ...
    def ask_claude(self, prompt, model='anthropic.claude-v2'):
        ...
        elif model == "anthropic.claude-instant-v1" or model == "anthropic.claude-v2" or model == "anthropic.claude-v2:1"  or model == "anthropic.claude-3-sonnet-20240229-v1:0" or model == "anthropic.claude-3-haiku-20240307-v1:0" or model == "anthropic.claude-3-opus-20240229-v1:0" or model == "anthropic.claude-3-5-sonnet-20240620-v1:0":
            body = json.dumps({
                "max_tokens": 20000,
    ...
    def list_models(self):
        bedrock= self.session_east_1.client(service_name='bedrock')
